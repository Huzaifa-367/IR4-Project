...
> **Depends on:** DOC-01 (settings mechanism, conventions), DOC-03 (`update-settings`; per-key gating), DOC-17 (`config_changed` audit). **Feeds:** every module that reads a tunable value — this is the single registry of all runtime configuration.
>
...
- **`update-settings`** — general operational tunables (timeouts, throttles, schedules, thresholds-config that aren't safety-critical).
- **`update-gas-thresholds`** — gas/CO₂ threshold values (safety-critical; DOC-11).
- **`update-settings` + confirmation** — retention and security keys (session timeout, lockout) require an extra confirm step in the UI.
...
| key | default | type | edit perm | used by |
|---|---|---|---|---|
| `auth.session_timeout_minutes` | `15` | int | update-settings + confirm | idle timeout |
| `auth.login_max_per_min` | `5` | int | update-settings | login throttle |
| `auth.lockout_threshold` | `10` | int | update-settings | lockout |
| `auth.lockout_minutes` | `15` | int | update-settings | lockout |
| `auth.password_min_length` | `12` | int | update-settings + confirm | password policy |
| `auth.require_2fa_for_admins` | `false` | bool | update-settings + confirm | 2FA gate |

### 4.3 Alerts (DOC-07)
| key | default | type | edit perm | used by |
|---|---|---|---|---|
| `alert.audible_enabled` | `true` | bool | update-alert-settings | audible master switch |
| `alert.warning_toast_seconds` | `10` | int | update-settings | toast dismiss |

### 4.4 Ingestion & real-time (DOC-08)
| key | default | type | edit perm | used by |
|---|---|---|---|---|
| `ingest.max_batch` | `1000` | int | update-settings | batch cap |
| `ingest.rate_per_minute` | `120` | int | update-settings | per-device throttle |
| `ingest.future_skew_seconds` | `300` | int | update-settings | clock-skew clamp |
| `ingest.backfill_after_seconds` | `600` | int | update-settings | backfill classification |
| `realtime.headcount_throttle_seconds` | `5` | int | update-settings | broadcast coalescing |
| `realtime.positions_throttle_seconds` | `5` | int | update-settings | broadcast coalescing |
| `realtime.gas_throttle_seconds` | `5` | int | update-settings | broadcast coalescing |
| `realtime.poll_fallback_seconds` | `30` | int | update-settings | poll fallback |

### 4.5 Hardware health (DOC-05)
| key | default | type | edit perm | used by |
|---|---|---|---|---|
| `health.reader_stale_minutes` | `5` | int | update-settings | markStale |
| `health.gas_stale_minutes` | `5` | int | update-settings | markStale |
| `health.sensor_stale_minutes` | `5` | int | update-settings | markStale (co2/env/gateway) |
| `health.edge_stale_minutes` | `3` | int | update-settings | markStale |
| `health.camera_stale_minutes` | `3` | int | update-settings | markStale (last_frame_at) |
| `health.gas_offline_escalate_minutes` | `30` | int | update-settings | gas-telemetry-lost escalation |

### 4.6 Tracking / RFID (DOC-09)
| key | default | type | edit perm | used by |
|---|---|---|---|---|
| `tracking.gate_debounce_seconds` | `60` | int | update-settings | gate flap prevention |
| `tracking.stationary_tag_minutes` | `15` | int | update-settings | stationary detection |
...
| key | default | type | edit perm | used by |
|---|---|---|---|---|
| `gas.hysteresis_margin_pct` | `5` | float | update-gas-thresholds | alarm auto-resolve |
| gas thresholds | see below | rows in `gas_thresholds` | update-gas-thresholds | evaluation |
...
| key | default | type | edit perm | used by |
|---|---|---|---|---|
| `env.thresholds_enabled` | `false` | bool | update-settings | (v1 has no env alarms) |

### 4.9 Equipment (DOC-13)
| key | default | type | edit perm | used by |
|---|---|---|---|---|
| `equipment.public_rate_limit_per_min` | `30` | int | update-settings | public QR page |
| `equipment.default_is_checkoutable` | `false` | bool | update-settings | new equipment default |
| `equipment.overdue_return_alerts` | `false` | bool | update-settings | overdue-return flag vs alert |
...
| key | default | type | edit perm | used by |
|---|---|---|---|---|
| `report.generation_day` | `sunday` | enum(mon…sun) | update-settings | scheduled generation |
| `report.generation_time` | `06:00` | time | update-settings | scheduled generation |
| `report.auto_publish` | `false` | bool | update-settings | auto-publish |
| `report.week_start` | `sunday` | enum(mon…sun) | update-settings | reporting week boundary |
| `report.completeness_threshold_pct` | `20` | int | update-settings | outage-note threshold |

### 4.11 Retention & backup (DOC-19)
| key | default | type | edit perm | used by |
|---|---|---|---|---|
| `retention.tag_readings_days` | `90` | int | update-settings + confirm | raw tag-read pruning |
| `retention.sensor_readings_days` | `180` | int | update-settings + confirm | raw gas/env pruning |
| `retention.exports_days` | `7` | int | update-settings | export-file cleanup |
...
| key | default | type | edit perm | used by |
|---|---|---|---|---|
| `dashboard.cache_seconds` | `8` | int | update-settings | summary cache |
...
- **Permission gating:** editing a `update-gas-thresholds` key without that permission → 403; general keys require `update-settings`; sensitive keys require confirm.
...
### Next document
**DOC-19 — Data Retention, Backup & End-of-Project:** on-read sensor aggregates (gas/env), raw-data pruning (never touching compliance tables), the data-volume math, encrypted daily backups (Spatie events → `AlertService`, no mail).