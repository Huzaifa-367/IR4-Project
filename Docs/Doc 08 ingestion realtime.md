...
| `POST /api/ingest/gas-readings` | `gas_detector` / `wifi_gateway` (CO₂ via edge RS485) | `{ event_uid, device_ref?, recorded_at, lel_pct?, h2s_ppm?, o2_pct?, co_ppm?, co2_ppm? }` — one endpoint carries all gas channels **and** CO₂; a reading includes whichever fields the sending device measures | DOC-11 |
...
- **Gas + CO₂ are one endpoint.** `/api/ingest/gas-readings` accepts a reading with any subset of the gas channels (`lel_pct, h2s_ppm, o2_pct, co_ppm`) and/or `co2_ppm`. A combined multi-gas detector sends the gas channels; a CO₂ sensor sends `co2_ppm`; a device that measures both sends both. DOC-11 stores/evaluates each present channel against its threshold. (Whether gas and CO₂ persist to one table or two is a DOC-11 storage decision; the *ingest wire contract* is a single endpoint.)
