...
>
> **Scope:** the **machine-data backbone** — the shared `/api/ingest/*` contract every device endpoint obeys (device auth, batching, idempotency, out-of-order/backfill handling, clock-skew, throttling), the ingest endpoints (tag-readings, ppe-violations [incl. fall], roi-violations, headcount-readings, gas-readings [incl. CO₂], environmental-readings, heartbeat), and the **Reverb** real-time layer (channel/event catalogue, authorization, throttling, poll fallback, LIVE/RECONNECTING). **Out of scope:** what each payload *means* and how it's processed into domain state (owned by DOC-09/10/11/12/23) — this doc defines how data *gets in* and how live updates *go out*.
...
| `POST /api/ingest/roi-violations` | `edge_compute` (camera AI) | `{ event_uid, camera_ref, roi_reference, event_type, detected_at, confidence, snapshot? }` — `event_type` = `roi_intrusion` | DOC-23 |
| `POST /api/ingest/headcount-readings` | `edge_compute` (camera AI) | `{ event_uid, recorded_at, count, camera_ref }` — absolute count for that camera's FOV; `zone_id` snapped from time-aware `camera_zone_bindings` (repositioning); live Total Manpower when `tracking.headcount_source=camera` | DOC-09 |
