...
- **Commissioning a pole:** operator creates Asset "Pole 2" (`is_mobile=true`) → adds its cameras (references `pole1-cam-n`, `pole1-cam-s` — this pole happens to have two) and its RFID reader (`pole1-reader`), plus any gas detector and edge unit fitted → issues a token per device → edge unit is configured with the tokens → heartbeats arrive → all tiles green on the health widget. A different pole with a single camera and no gas detector is registered exactly the same way, just with fewer children.
