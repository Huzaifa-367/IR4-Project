...
### Next document
**DOC-16 — Dashboard & Display Mode:** the single `/api/dashboard/summary` aggregate, the role-aware widget grid (PM KPI variant), the 55″ wall as a screen-cast of `/dashboard`, the shared occupancy / reading tables, and the permission-driven navigation/sidebar.