...
# Point timesyncd at the pole SCC (.40 from IR4_BASE_URL), set APP_TIMEZONE,
# and enable time-wait-sync so agents do not stamp recorded_at as 1970 on boot.
ensure_time_sync() {
  load_secret_env
  local tz="${APP_TIMEZONE:-Asia/Riyadh}"
  local ntp_host=""
  if [[ -n "${IR4_BASE_URL:-}" ]]; then
    ntp_host="$(
      python3 - "${IR4_BASE_URL}" <<'PY'
...
PY
  ...
  if [[ -z "${ntp_host}" ]]; then
    echo "==> Time sync: IR4_BASE_URL missing — skip NTP server pin" >&2
  else
    echo "==> Time sync: NTP=${ntp_host} timezone=${tz}"
    install -d -m 0755 /etc/systemd/timesyncd.conf.d
    # Drop-in wins over NVIDIA nv-fallback-ntp.conf (public pools — useless on-prem).
    cat > /etc/systemd/timesyncd.conf.d/ir4-edge.conf <<EOF
# Generated by ir4-edge deploy — SCC chrony only (no public NTP).
[Time]
NTP=${ntp_host}
FallbackNTP=${ntp_host}
EOF
    chmod 0644 /etc/systemd/timesyncd.conf.d/ir4-edge.conf
    # Neutralize NVIDIA public FallbackNTP if present.
    if [[ -f /etc/systemd/timesyncd.conf.d/nv-fallback-ntp.conf ]]; then
      cat > /etc/systemd/timesyncd.conf.d/zz-ir4-override-nvidia.conf <<EOF
[Time]
FallbackNTP=${ntp_host}
EOF
      chmod 0644 /etc/systemd/timesyncd.conf.d/zz-ir4-override-nvidia.conf
    fi
    if grep -qE '^\s*#?\s*NTP=' /etc/systemd/timesyncd.conf 2>/dev/null; then
      sed -i -E "s|^#?NTP=.*|NTP=${ntp_host}|" /etc/systemd/timesyncd.conf
    fi
  fi
  if command -v timedatectl >/dev/null 2>&1; then
    timedatectl set-timezone "${tz}" 2>/dev/null || \
      echo "==> timedatectl set-timezone ${tz} failed (non-fatal)" >&2
    timedatectl set-ntp true 2>/dev/null || true
  fi
  systemctl enable systemd-timesyncd.service 2>/dev/null || true
  systemctl restart systemd-timesyncd.service 2>/dev/null || true
  # Blocks WantedBy/After=time-sync.target until clock is NTP-synced.
  systemctl enable systemd-time-wait-sync.service 2>/dev/null || true
  systemctl start systemd-time-wait-sync.service 2>/dev/null || true
}

# Write ir4-gas-agent.service and ir4-rfid-agent.service from templates.
render_systemd_units() {
  chmod +x "${EDGE_ROOT}/deploy/scripts/wait_for_ir4.sh" 2>/dev/null || true
  ensure_time_sync
